...
const findFilenameAndExtension = function(files) {
  const filenameRegex = /^(.+)\.(pdf|jpe?g|png)$/gmi;
  let match;
  const matches = [];
  while (match = filenameRegex.exec(files)) matches.push([match[1], match[2]]);
  // example match: [ 
  //   'kittens.jpeg',
  //   'kittens',
  //   'jpeg',
...
const replaceSWordsMatchingCase = function(string) {
  return string.replace(/\b(s)\w+\b/gi, (match, first_letter) => `${first_letter}-word`);
}
